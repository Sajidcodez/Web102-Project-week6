...
function List({ searchValue, setSearchValue, setTopBrewery }) {
  ...
  // Filter breweries based on selected type
  const filteredBreweries = breweries?.filter(brewery => {
    if (filterType === "all") return true;
    return brewery.brewery_type === filterType;
  });
  ...
  return (
    <div className='flex flex-col items-center justify-center h-auto pb-10 bg-purple-200/20 p-10 rounded-xl mr-6'>
        ...
        <div className='flex gap-4 mt-4'>
            <label className='text-white text-xl'>Filter by Type:</label>
            <select 
                value={filterType} 
                onChange={(e) => setFilterType(e.target.value)}
                className='pl-2 p-1 text-xl rounded-md'
            >
